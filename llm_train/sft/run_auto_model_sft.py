...
import datasets
import torch
from data import build_instruction_dataset, DataCollatorForSupervisedDataset
import transformers
...
class ModelArguments:
    ...
    use_cache: Optional[str] = field(
        default=True, metadata={"help": "use_cache=True 这个参数的作用是启用或禁用缓存机制。"
                                        "当设置为 True 时，模型会将计算结果存储在缓存中，以便在后续训练过程中重复使用，从而提高训练速度。"
                                        "但是，这也会增加内存的使用量。因此，在选择是否使用缓存时，需要根据你的硬件配置和训练需求进行权衡。"
                                        "如果你的机器内存充足，并且希望提高训练速度，可以设置 use_cache=True。"
                                        "反之，如果内存资源有限，那么可能需要设置 use_cache=False 来减少内存使用。"}
    )
...
def main():
    ...
    # v100 跑 qwen 时显存不够可以设置此参数降低显存使用
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            files = [os.path.join(path, file.name) for file in path.glob("yzy_sft_new.json")] + \
                    [os.path.join(path, file.name) for file in path.glob("yzy_sft_new.jsonl")]
    ...
    elif data_args.validation_split_percentage is not None:
        if data_args.validation_split_percentage is not None:
            lm_datasets = train_dataset.train_test_split(test_size=data_args.validation_split_percentage)
            train_dataset = lm_datasets['train']
            eval_dataset = lm_datasets["test"]
            logger.info(f"split train val by validation_split_percentage {data_args.validation_split_percentage}")
            logger.info(f"Num train_samples  {len(train_dataset)}")
            logger.info(f"Num eval_samples  {len(eval_dataset)}")
